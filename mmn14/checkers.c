...
#define CHECK_REGISTER if(*(token)=='@') {ok=1;if(*(token+1)!='r' || (*(token+2))>'8' || (*(token+2))<'1' ||(!isspace(*(token+3))&&*(token+3)!=','&&*(token+3))) \
...
#define CHECK_END if(token && (*token!='\n' && *token!='\0')) {ERROR=END_ERROR;EXIT}
...
line_data checkLine(char line[LINE_SIZE+1],FILE * file,int lineNumber){
    ...
    char t[LINE_SIZE],p[LINE_SIZE],*token,*param,skipper;
    line_data lineData;
    ...
    if (*line==COMMENT_SIGN){/*check comment*/
        EXIT
    }
    for (i = 0; token[i] != '\n' && token[i] != '\t' && token[i] != '\0' && token[i] != ' '; i++)
        if(token[i]==':')
    ...
    switch (cmd) {
        ...
        case 16:/*data*/
            ...
            while (*token!='\n' && token) {
                ...
                CHECK_NUMBER
                if (ok)
                /*{if (!(DATA_PARAM = realloc(DATA_PARAM,(i*sizeof(int))))){EXIT};*/
                    {DATA_PARAM[NUMBER_OF_DATAS]=strtol(param,NULL,10);}
                else
                    {ERROR = ILLEGAL_PARAM;EXIT}
                DELETE_SPACES
        ...
        case 17:/*string*/
            ...
            CHECK_APOSTROPHES
            /*STRING_PARAM = (char *)malloc(i * sizeof(char));*/
...
int checkNumber (char ** token) {
    ...
    (*token)++;
    while (**token != ',' && **token != '\0' && **token != ' ' && **token != '\t' && **token!='\n'){
        if (!isdigit(**token))
            return ILLEGAL_NUMBER;
        (*token)++;}
        return 1;
...
int checkLabelParam(char token[LINE_SIZE]) { /*if label ok return j if not label return 0 if error return -1*/
    int i, j;
    i = 0;
    if (isalpha(token[0])) {
